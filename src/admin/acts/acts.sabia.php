		...
		switch ($_GET['act']) {
			case 'showupd':
				try {
			    	...
			    	$qry_sabia = $conn->query("SELECT id, pergunta, resposta, ativo FROM sabia WHERE id = $id") or trigger_error("27005 - " . $conn->error);

					if ($qry_sabia && $qry_sabia->num_rows > 0) {
						$dados = "";
		    			while($vcsabia = $qry_sabia->fetch_object()) {
		    				$dados = '{"id" : "' . $vcsabia->id . '", "pergunta" : "' . $vcsabia->pergunta . '", "resposta" : "' . $vcsabia->resposta . '",  "ativo" : "' . $vcsabia->ativo . '"}';
		    			}
			...
			case 'add':
				try {
					...
					if(isset($_POST) && !empty($_POST) && $_POST["pergunta"]) {
						$isValid = true;
						$errMsg = "";

						if(!isset($_POST["pergunta"]) || empty($_POST["pergunta"])) {
							$errMsg .= "Pergunta (Informe sua pergunta)";
							$isValid = false;
						}
						

						if(!isset($_POST["resposta"]) || empty($_POST["resposta"])) {
							$errMsg .= "Informe a resposta";
							$isValid = false;
						}
						...
						if(!$isValid) {
							...
							$qry_sabia = "INSERT INTO sabia (pergunta, resposta, ativo) VALUES ('" . $pergunta . "','" . $resposta . "', '" . $ativo . "')";

							if ($conn->query($qry_sabia) === TRUE) {
								$conn->commit();
								echo '{"succeed": true}';
							} else {
						        throw new Exception("Erro ao inserir o evento: " . $qry_sabia . "<br>" . $conn->error);
							}
						}
					}
					else {
					...
					if(isset($_POST) && !empty($_POST) && $_POST["pergunta"]) {
						...
						if(!$isValid) {
							...
							if ($conn->query($qry_sabia) === TRUE) {
						        ...
						        throw new Exception("Erro ao alterar o evento: " . $qry_sabia . "<br>" . $conn->error);
							}
						}
					}
					else {
			...
			try {
				...
				if(!isset($_GET['id']) || empty($_GET['id'])) {
					echo '{"succeed": false, "errno": 27020, "title": "Parâmetro não encontrado!", "erro": "Parâmetro do ID do sabia não enviado! Favor contatar o administrador mostrando o erro!"}';
					exit();
				}

				$id = $_GET['id']; // $_SESSION["fake_id"];

				$qrydel_sabia = "DELETE FROM sabia WHERE id = $id";
					if ($conn->query($qrydel_sabia) === TRUE) {
						$conn->commit();
						echo '{"succeed": true}';
					} else {
				        throw new Exception("Erro ao remover o servico: " . $qrydel_sabia . "<br>" . $conn->error);
					}
